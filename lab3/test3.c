...
int kbd_test_scan(unsigned short ass) { //Depending on whether its argument is zero or not, it should use the IH written in C or the IH written in assembly.
...
int kbd_test_leds(unsigned short n, unsigned short *leds) {
	...
	/*usar a função do timer do lab2 para quando passar um segundo, em vez de imprimir os segundos que passaram, processar um dos valores do vetor */
	int ipc_status;
	message msg;
	unsigned long byte;
	int id;
...
int kbd_test_timed_scan(unsigned short n) {
	/* should exit not only when the user releases the ESC key, but also if it does not receive a scancode for a number seconds equal to its argument, n. */
