...
static int proc_args(int argc, char **argv)
{
	...
	if (strncmp(argv[1], "kbscan_test", strlen("kbscan_test")) == 0) {
		...
		vetor = (unsigned short*) malloc(elementos*sizeof(unsigned short)); //Alocar memória para o vetor
		for(i=0;i< elementos;i++){
			if (parse_ulong(argv[i+2],10) == ULONG_MAX) return 1;
			else vetor[i] = parse_ulong(argv[i+2],10); }// Guardar no vetor os argumentos
		
		//Testar se todos os argumentos são 0,1 ou 2
		for(i=0;i< elementos;i++){ if(vetor[i] < 0 || vetor[i] > 2){
						printf("%d is not a valid LED!\n",vetor[i]);
						return 1;}
					}
		
		printf("kdb::kbd_test_leds(%d,'",elementos);	
		for(i=0;i<elementos;i++){ if(i==elementos-1) printf("%u')\n",vetor[i]);
					else printf("%u,",vetor[i]);}
